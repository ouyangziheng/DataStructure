...
class Edge {
 public:
  Edge(int nodeIndexA = 0, int nodeIndexB = 0, int weightValue = 0)
      : node_index_a(nodeIndexA),
        node_index_b(nodeIndexB),
...
class Mymap {
 ...
 private:
  ...
  void breadthFirstTraverseImpl(vector<int> pre_vec) {
    ...
    for (int i = 0; i < pre_vec.size(); i++) {
      for (int j = 0; j < capacity; j++) {
        ...
        if (val != 0) {
          if (node_array[j].is_visited) continue;
  ...
  int getMinEdge(const vector<Edge> &edgeVec) {
    ...
    for (int i = 0; i < (int)edgeVec.size(); i++) {
      if (edgeVec[i].is_selected) continue;
 ...
 public:
  ...
  // 将顶点访问设置默认
  void resetNode() {
    for (int i = 0; i < node_count; i++) node_array[i].is_visited = false;
  }
  // 设置权重-有向图
  ...
  void depthFirstTraverse(int index) {
    ...
    for (int i = 0; i < capacity; i++) {
      ...
      if (val != 0) {
        if (node_array[i].is_visited) continue;
  ...
  void dijkstra(int start) {
    ...
    while (!pq.empty()) {
      ...
      if (visited[currentNode]) continue;  // 如果该节点已访问，则跳过
